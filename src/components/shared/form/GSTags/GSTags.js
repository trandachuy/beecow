...
const GSTags = React.forwardRef((props, ref) => {
    ...
    const handleOnblur = (event) => {
        ...
        if (props.ignoreMatchRegex) {
            inputValue = inputValue.replace(props.ignoreMatchRegex, '')
        }
        if (!inputValue) {
            setStInputValue('')
            return;
        ...
        setStInputValue('')
        setStValue([...value, createOption(inputValue)])
        if (props.onChange) props.onChange([...value, createOption(inputValue)])
    }

    const handleInputChange = (inputValue) => {
        setStInputValue(inputValue)
        if(props.isSearch && inputValue) {
           if (props.onFilter) props.onFilter(inputValue, false)
        } else if (!inputValue) {
    ...
    const {placeholder, defaultValue, confirmKeys, isClearable, onChange, style,maxItemTextLength,maxLength,ignoreMatchRegex, className, ...other} = props
    return (
            <>
                <CreatableSelect
                    {...other}
...
GSTags.propTypes = {
    ...
    ignoreMatchRegex: PropTypes.any,
