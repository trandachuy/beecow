...
const ProductSearch = props => {
    ...
    const onInputSearch = (event) => {
        ...
        dispatch(OrderInStorePurchaseContext.actions.setSearchProductText(keyword))
    ...
    const selectProduct = (product) => {
        ...
        dispatch(OrderInStorePurchaseContext.actions.addNewProduct(clonedProduct))
        // temp product list
    ...
    function handleScan(data) {
        ItemService.getProductSuggestionByName(
            ...
            true,
            storeBranch.value,
            {platform: 'IN_STORE'}).then(result => {
            if (result.data.length) {
                selectProduct(separateModel(result.data).shift())
            } else {
                dispatch(OrderInStorePurchaseContext.actions.barCodeNotFound(new Date().getTime()))
                dispatch(OrderInStorePurchaseContext.actions.barcodeScanned(data))
            }
        }).catch(e => {
            GSToast.commonError()
        })
    ...
    return (
        <>
            ...
            <div className="order-instore-purchase">
                {/* Text-box search */}
                <div className="d-flex align-items-center">
                    ...
                    <div style={{marginLeft: "10px"}}>
                        <BarCodeScanButton handleScan={handleScan} handleError={handleError}></BarCodeScanButton>
                    </div>
                </div>
                ...
                {   
                    <div 
                        hidden={!showSearch}
                        style={{display: 'block', top: '12.5px'}}
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="product-item-row__product-inventory">
                                            <span>{CurrencyUtils.formatMoneyByCurrency(product.price, CurrencyUtils.getLocalStorageSymbol())}</span>
