...
import './OrderInZaloProductList.sass';
...
const OrderInZaloProductList = props => {
    ...
    const tabIndex = useRecoilValue(OrderInZaloRecoil.currentOrderIndexState)
    const isCheckedAll = state.productList.filter(p => p.checked).length === state.productList.length && state.productList.length > 0

    useEffect(() => {
        OrderInZaloContextService.dispatchUpdateProductList(state, dispatch, state.productList, storeBranch)
    }, [state.checkWholeSalePrice, state.profileId])

    const onClickCheckAll = () => {
        if (isCheckedAll) {
            dispatch(OrderInZaloContext.actions.uncheckAllProduct())
        } else {
            dispatch(OrderInZaloContext.actions.checkAllProduct())
        }
    }
    const openRemoveConfirmModal = (productId) => {
        const handleOkCallback = () => {
            dispatch(OrderInZaloContext.actions.removeProduct(productId))
            const productListAfterRemove = [...state.productList].filter(p => p.id !== productId)
            OrderInZaloContextService.dispatchUpdateProductList(state, dispatch, productListAfterRemove, storeBranch)
            dispatch(OrderInZaloContext.actions.clearInsufficientErrorGroup(productListAfterRemove))
        }
    ...
    return (
        <>
            ...
            <div className="order-in-zalo-cart-product-list gs-atm__scrollbar-1 h-100">
                <GSTable>
                    <thead className="">
                    <tr>
...
OrderInZaloProductList.propTypes = {};
const ProductRow = (props) => {
    ...
    const updateProductHasWholeSalePrice = (data) =>{
        return dispatch(OrderInZaloContext.actions.updateProductHasWholeSalePrice(data));
    }
    const onClickCheck = () => {
        // dispatch(OrderInZaloContext.actions.checkToggleProduct(props.data.id))
        onChangeProduct({
            checked: !props.data.checked
        })
    }
    const onClickRemove = () => {
        dispatch(OrderInZaloContext.actions.removeProduct(props.data.id))
        const productListAfterRemove = [...state.productList].filter(p => p.id !== props.data.id)
        OrderInZaloContextService.dispatchUpdateProductList(state, dispatch, productListAfterRemove, storeBranch)
        dispatch(OrderInZaloContext.actions.clearInsufficientErrorGroup(props.data))
    }
    const onChangeProduct = (value) => {
        dispatch(OrderInZaloContext.actions.modifyProduct({
            ...props.data,
            ...value
        }))
        dispatch(OrderInZaloContext.actions.clearInsufficientErrorGroup(props.data))
        ...
        >
            ...
            <td style={{width:"200px"}}>
                <div className="d-flex justify-content-start align-items-center">
                    ...
                    />
                    <div className="ml-3 order-in-store-purchase-cart-product-list__product-name">
                        <h6>
                            {props.data.name}
                        </h6>
...
export default OrderInZaloProductList;
