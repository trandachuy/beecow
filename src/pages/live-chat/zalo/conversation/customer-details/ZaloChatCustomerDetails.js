...
import './ZaloChatCustomerDetails.sass'
...
const ZaloChatCustomerDetails = props => {
        ...
        if(stFbUserId){
            ...
            if(stMatch.params.hasPin && stMatch.params.hasPin === true ){
                ...
                refShowComfirm.current.openModal({
                    messages: (<GSTrans t="page.livechat.customer.details.pin.pinned.zalo">
                                    This customer is already pinned with other Zalo user. <strong>Do you want to continue connect?</strong>
                                </GSTrans>),
                    okCallback: () => {
                        // link facebookec user with the customer profile
                        beehiveService.linkZaloUserToCustomerProfile(profileId, stFbUserId, "PIN").then(res => {
                            stIsPin(true);
                ...
                // link zalo  user with the customer profile
                beehiveService.linkZaloUserToCustomerProfile(profileId, stFbUserId, "PIN").then(res => {
                    stIsPin(true);
                    setStIsPinning(false)
                }).catch( e => {
    ...
    const unpin = (profileId) => {
        ...
        if(stFbUserId){
            setStIsPinning(true)
            // link facebookec user with the customer profile
            beehiveService.linkZaloUserToCustomerProfile(profileId, stFbUserId, "UNPIN").then(res => {
                stIsPin(false);
                setStIsPinning(false)
            }).catch( e => {
                stIsPin(true);
    ...
    useEffect(() => {
        ...
        if (state.currentConversation) {
            const fbUserId = ZaloChatUtils.getClientInfoFromConversation(state.currentConversation).id
    ...
    const getUserLinkWithCustomerProfile = (fbUserId) => {
        ...
        beehiveService.getZaloUserLinkWithCustomerProfile(fbUserId).then(res => {
            setStMatch({
                ...stMatch,
                params: {
                    ...stMatch.params,
    ...
    const changeCustomerProfileInfo = () => {
        dispatch(ZaloChatConversationContext.actions.addNewCustomer(true));
    };
                                    ...
                                    {stItemList.map(item => {
                                        return (
                                            <div className="gs-mobile-list__row"
                                                 key={'m' + item.id}
                                                 onClick={() => onClickCustomerRow(item.id, item.userId, item.saleChannel, item.zaloUsers ? item.zaloUsers.length : 0)}>
                                                <div className="mobile-customer-profile-row__info">
                                                    <div className="mobile-customer-profile-row__left">
                                                        ...
                                                        <div className="pin-cnt">{item.zaloUsers ? item.zaloUsers.length : 0} <GSTrans t="page.livechat.customer.details.pinned">pinned</GSTrans>
                                                        </div>
                                                    </div>
...
ZaloChatCustomerDetails.propTypes = {
...
export default ZaloChatCustomerDetails;
