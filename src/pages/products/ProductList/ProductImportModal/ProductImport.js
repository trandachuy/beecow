...
import BranchesSelector, { BranchContext, BRANCH_SELECT_MODE_IMPORT } from '../../../../components/shared/BranchesSelect/BranchesSelector';
...
const ProductImport = props => {
    ...
    const onClickImport = () => {
        ...
        ItemService.importItemList(stFile, selectedBranches)
            .then(result => {
    ...
    const onClickDownloadTemplate = () => {
        ...
        ItemService.getItemImportTemplate().then(res => {
        const url = window.URL.createObjectURL(new Blob([res]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', 'item-template-import.xlsx');
    ...
    return (
        <>
            ...
            <Modal isOpen={stIsOpen} {...other} centered={true} fade={true} className="item-list-import-modal">
                <ModalHeader>
                    <GSTrans t="productList.modal.importModal.title"/>
                </ModalHeader>
                <ModalBody>
