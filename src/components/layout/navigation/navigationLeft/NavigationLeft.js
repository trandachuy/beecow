...
const NavigationLeft = props => {
    ...
    return (
        <div {...other} className={['navigation-left', !stIsOpen? 'navigation-left--closed':'', className].join(' ')}
            onClick={onClosePanel}
        >
            <div  className={["left-panel-wrapper", !stIsOpen? 'left-panel-wrapper--closed':''].join(' ')}
            ...
            <div className="logo">
                ...
                <Navigation active={active} className="left-panel" collapsible={true}/>
            </div>
        </div>
    );
};
