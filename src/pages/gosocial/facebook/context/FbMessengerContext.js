...
const initState = {
    ...
    // the current page of user
    currentConversation: null,

    // the current page of seller
    currentPage: null, // not use
    fbUserDetail: {}, // use this
    ...
    fetchMode: FETCH_MODE_TYPE.REALTIME,
...
const actions = {
    ...
    changeCurrentPage: (page) => {
        return ContextUtils.createAction('CHANGE_CURRENT_PAGE', page)
    },
    secondsTick: () => {
    ...
    disconnectWebhook: (connection) => {
        return ContextUtils.createAction('DISCONNECT_WEBHOOK', connection)
    },
    ...
    setUserId: (userId) => ContextUtils.createAction('SET_USER_ID', userId),
    ...
    setfbUserDetail: (item) => ContextUtils.createAction('SET_FB_USER_DETAIL', item),
    setUnreadConversationNumber: (number) => ContextUtils.createAction('SET_UNREAD_FB_NUMBER', number),
    setOpenOrder: (isOpen) => ContextUtils.createAction('SET_OPEN_ORDER_TAB', isOpen),
    setNewOrderResponse: (response) => ContextUtils.createAction('SET_NEW_ORDER_RESPONSE', response),
    clearNewOrderResponse: () => ContextUtils.createAction('CLEAR_ORDER_RESPONSE'),
...
const reducer = (state, action) => {
    console.debug('FbMessengerContext', state, action)
    switch (action.type) {
        case 'CLEAR_ORDER_RESPONSE':
            return {
                ...state,
                newOrderResponse: null
            }
        case 'SET_NEW_ORDER_RESPONSE':
            return {
                ...state,
                newOrderResponse: action.payload
            }
        ...
        case 'CHANGE_CURRENT_CONVERSATION':
            return {
                ...state,
                fbUserId: action.payload?.senders?.data[0]?.psid,
                fbUserName: action.payload?.senders?.data[0]?.name,
        ...
        case 'SET_FB_USER_DETAIL': {
            return {
                ...state,
                fbUserDetail: action.payload,
            }
        }
        case 'SET_UNREAD_FB_NUMBER': {
            return {
                ...state,
                unreadConversationNumber: action.payload,
            }
        }
        case 'SET_OPEN_ORDER_TAB': {
            return {
                ...state,
                hasOpenOrder: action.payload,
            }
        }
...
export const FbMessengerContext = {
    context,
