...
export default class MenuAddModal extends Component {
    ...
    constructor(props) {
        ...
        this.isChangedParentMenu = this.isChangedParentMenu.bind(this);
    ...
    loadUrlLinksType(menuItem) {
        ...
        if (menuItem.dataType === MENU_ITEM_TYPE.COLLECTION_SERVICE || menuItem.dataType === MENU_ITEM_TYPE.COLLECTION_PRODUCT) {
            ...
            // blog
            this.getBlogByStore(this.currentPage, this.SIZE_PER_PAGE);
            this.getBlogDetail(menuItem.dataValue);
    ...
    addMenuItem(values, items, order) {
        ...
        items.push(data);
        this.openAlertModal(AlertModalType.ALERT_TYPE_SUCCESS, i18next.t('component.menu.item.add.success'), items)
    }
    editMenuItem(values, items, order) {
        if (this.isChangedParentMenu()) {
            ...
            if (this.state.currentMenuItem.parentId > 0) {
                ...
                if (parentMenu.menuItems.length < 2) {
                    ...
                    // re-order children menuItem
                    items = this.reOrderMenuItems(parentMenu.menuItems, this.state.currentMenuItem.id, 0);
                }
            }

            if (this.state.parentMenuItemSelected.id === -1) {
                let menuItems = this.reOrderMenuItems(this.state.menuItems, -1, 1)
                this.state.currentMenuItem.parentId = 0;
                this.state.currentMenuItem.order = 0;
                this.state.currentMenuItem.level = 0;

                let ids = new Set(menuItems.map(ite => ite.id));
                items = [...items.filter(ite => !ids.has(ite.id)), ...menuItems];
                ...
                let ids = new Set(result.map(ite => ite.id));
                items = [...items.filter(ite => !ids.has(ite.id)), ...result];
        ...
        this.openAlertModal(AlertModalType.ALERT_TYPE_SUCCESS, i18next.t('component.menu.item.edit.success'), items)

    }
    reOrderMenuItems(menuItems, condition, reOrderIdx) {
        return menuItems.filter((ite) => {
            if (ite.id !== condition) {
                ite.order = reOrderIdx;
                reOrderIdx++;
                return ite;
            }
        });
    ...
    isChangedParentMenu() {
        ...
        if (this.state.currentMenuItem.parentId === this.state.parentMenuItemSelected.id) {
            return false;
        }
        if (this.state.currentMenuItem.parentId === 0 && this.state.parentMenuItemSelected.id === -1) {
            return false;
        }
        return true;
    }
    ...
    // for blog
    getBlogByStore(page, size) {
        clearTimeout(this.timeOut);
        beehiveService.getBlogCategories(page, size)
            .then(result => {
                const newData = result.map(category => ({ name: category.title, id: category.id }));
                let data = this.state.data.slice();
                if (page === 0) {
                    data = []
                }
    ...
    getBlogDetail(value) {
        this.setState({ urlLinkTypeSelected: { name: "Default Blog", value: 0 } });
    }
    ...
    /**
     * This function generate unique id as integer with 32 bits.
     */
    randomId() {
